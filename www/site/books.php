...
function constroi_tabela_eventos($numerodedestaques, $colunas, $pagina){
	...
	else $query = "SELECT eventos.cd, eventos.nomes, eventos.data, eventos.imagem_destaque, tipodeevento.tipo FROM eventos, tipodeevento WHERE (eventos.tipo=tipodeevento.cd) AND (eventos.data < " . mktime() . ") AND (eventos.status = 4) " . $filtro . " ORDER BY data DESC" . $query_limit;

	require("conectar_mysql.php");
	$result = mysql_query($query) or die("Erro de conexão ao banco de dados: " . mysql_error() . $query);
	?>
	<div style="padding:20px;">
	<center><strong>Books Fotográficos<br></strong></center><hr />
	<?
		...
		while($evento = mysql_fetch_array($result, MYSQL_ASSOC)){
			...
			<td width="33%" align="center" valign="top">
				<table width="100%" height="130" border="0">
					<tr>
						<td align="center" valign="top" height="50%"><a href="ver_evento-book.php?cd=<?=$evento["cd"]?>"><img border="0" src="../<?=$imagem[0]?>"></a></td>
					</tr>
					<tr>
