...
add_action( 'save_post', 'dt_home_youtube_save' );
...
*/
		...
		add_meta_box(
			'dt_page_box-youtube',
			__( 'Opcoes para Youtube', 'dt' ),
			'dt_home_slider_youtube_options',
			'page',
			'side'
		);
	...
	<div class="showhide">
		...
		<?php if( $posts ):	foreach( $posts as $p ): ?>
			<label style="width: <?php echo $img_w; ?>px;display: inline-block;margin-bottom: 3px;">
				<img width="<?php echo $img_w; ?>" height="<?php echo $img_h; ?>" src="<?php echo esc_url($p->dt_thumbnail); ?>" title="<?php echo $p->dt_info.$counts[$p->ID]; ?>"/><br/>
	...
	<div class="showhide">
		...
		<?php if( $posts ):	foreach( $posts as $p ): ?>
			<label style="width: <?php echo $img_w; ?>px;display: inline-block;">
				<img width="<?php echo $img_w; ?>" height="<?php echo $img_h; ?>" src="<?php echo $p->dt_thumbnail; ?>" title="<?php echo $p->dt_info.$counts[$p->ID]; ?>"/><br/>
				<div style="height: 30px;overflow: hidden;background-color: #D7D7D7;">
...
// VIDEO METABOX
function dt_home_slider_youtube_options( $post ) {
	// NAME OF THE BOX !
	$box_name = 'youtube';
	...
	$data = wp_parse_args(
		$data,
		array(
			'dt_youtube_autoplay'	=> false,
			'dt_youtube_loop'		=> false,
			'dt_youtube_id'	    	=> ""	
		)
	);

	$video_id = isset( $data['dt_youtube_id'] )?trim( $data['dt_youtube_id'] ):'';
	...
	?>
	<p>
		<label for="dt_youtube_id"><?php _e('ID do Video no Youtube', LANGUAGE_ZONE ); ?></label>
		<input type="text" id="dt_youtube_id" name="dt_youtube_id" value="<?php echo esc_attr($video_id); ?>" size="46" />
	</p>
	<p>
		<label for="dt_youtube_autoplay">
			<input type="checkbox" id="dt_youtube_autoplay" name="dt_youtube_autoplay"<?php checked($data['dt_youtube_autoplay']); ?>/>
			<?php _e("Autoplay", LANGUAGE_ZONE ); ?>
		</label>
	</p>
	<p>
		<label for="dt_youtube_loop">
			<input type="checkbox" id="dt_youtube_loop" name="dt_youtube_loop"<?php checked($data['dt_youtube_loop']); ?>/>
			<?php _e("Repetir", LANGUAGE_ZONE ); ?>
		</label>
	</p>
	<?php
}
...
// VIDEO SAVE FUNK
function dt_home_youtube_save( $post_id ) {
	// NAME OF THE BOX !
	$box_name = 'youtube';
	...
	$mydata['dt_youtube_id'] =  $_POST['dt_youtube_id'];
	$mydata['dt_youtube_autoplay'] = isset($_POST['dt_youtube_autoplay']);
	$mydata['dt_youtube_loop'] = isset($_POST['dt_youtube_loop']);
