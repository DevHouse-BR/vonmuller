...
?>    
  ...
  <div id="respond">
    <div class="header"><?php echo __('Share a comment', LANGUAGE_ZONE); ?>:</div>
    
    <?php if (is_user_logged_in()) { ?>
      <small><?php echo __('You are currently logged in as', LANGUAGE_ZONE); ?> <a href="<?php echo home_url('/').'author/'.$current_user->user_login.'/'; ?>"><?php echo $current_user->user_login; ?></a></small>
    <?php } else { ?>
    <small><?php echo __('Your email address will not be published. Required fields are marked', LANGUAGE_ZONE); ?> <span class="required">*</span></small>
    <?php } ?>
    ...
    <form action="<?php echo home_url('/'); ?>wp-comments-post.php" method="post" id="commentform" class="uniform">
     ...
     <a href="#" id="submit" class="go_button do_add_comment subm_comm go_add_comment" title=""><span>
         <i></i>
