       ...
       <div class="plugin_embed form-actions clearfix">
       	<ul class="jac-wrapper-actions">			        								    
    	    ...
    	    <?php if($enableAfterTheDeadline){?>
		    	<li class='form-act-spell'><a href="javascript:check_atd('<?php echo $prefix;?>')" title="<?php echo JText::_("Check Spelling"); ?>"><span id="checkLink<?php echo $prefix;?>"><?php echo JText::_("Check Spelling"); ?></span></a></li>			         
		    <?php } ?>
			    	    	            
		    <?php if($enableYoutube){ ?>
		    	<li class="form-act-utube">			        
	                <a href="javascript:open_youtube('<?php echo $itemID;?>');" class="plugin" title="<?php echo JText::_("Embed Video");?>"><span><?php echo JText::_("Embed Video");?></span></a>
	            </li>
		    <?php } ?>
					    
		    <?php if($isAttachImage && ($allowUploadSubscription || $listFiles)){?>
		    	<li class="form-act-attach">						    				    		
		    		<a href="javascript:openAttachFile('<?php echo $itemID;?>');" class="plugin" title="<?php echo JText::_("Attach File");?>"><span><?php echo JText::_("Attach File");?></span></a>							    		
		    	</li>
		    <?php }?>
...
<!-- BEGIN - Text area  -->    
	<div id="jac-container-textarea<?php echo $prefix; ?>" class="form-comment clearfix">		
		...
		?>															  
		<textarea <?php if($isEnableAutoexpanding){?>style="overflow-y: hidden;"<?php }?> onblur="checkWordLength(this.value,'<?php echo $textAreaID;?>', 'jaCountText<?php echo $prefix;?>')" class="field textarea jac-new-comment jac-expand-field jac-inner-text" rows="12" cols="80" tabindex="1" id="newcomment<?php echo $prefix; ?>" name="newcomment"><?php if($prefix == ""){echo JText::_("TEXT_COMMENT");}else{echo $contentComment;}?></textarea>
		<?php if($enableBbcode){?>
	...
	<?php if($prefix == ""){?>
		<input type="hidden" id="jac_hid_text_comment" value="<?php echo JText::_("TEXT_COMMENT");?>"/>
	<?php }?>
<!-- END - Text area  -->									        